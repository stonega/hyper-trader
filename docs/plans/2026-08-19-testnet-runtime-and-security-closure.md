...
| M4 | Late-result and single-flight races | Deterministic deferred-auth/SecureStore tests prove epoch and binding fences discard every late result. Physical tests prove the expected native authentication sheet can complete only after active focus returns, while task-switch, background, notification/control-center, and interrupted-dialog races produce a locked session and no signature. |
