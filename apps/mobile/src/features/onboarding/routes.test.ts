...
describe("onboarding route contract", () => {
  ...
  test("opens persisted setup intent in Setup and read-only choice in Trade", () => {
    expect(SETUP_ROUTE).toBe("/setup");
    expect(welcomeChoiceRoute("setup")).toBe(SETUP_ROUTE);
    expect(welcomeChoiceRoute("read_only")).toBe(TRADE_ROUTE);
  });
